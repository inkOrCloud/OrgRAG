...
from lightrag.api.passwords import (
...
def _make_db(
    path: str, username: str = "alice", password: str = "old_pass", role: str = "admin"
):
...
def test_reset_password_updates_db(tmp_path, capsys):
    ...
    _run_reset_password(
        ["alice", "--password", "new_pass", "--working-dir", str(tmp_path)]
    )
...
def test_reset_password_unknown_user_exits(tmp_path):
    ...
    with pytest.raises(SystemExit) as exc_info:
        _run_reset_password(
            ["nobody", "--password", "x", "--working-dir", str(tmp_path)]
        )
...
def test_reset_password_missing_db_exits(tmp_path):
    with pytest.raises(SystemExit) as exc_info:
        _run_reset_password(
            ["alice", "--password", "x", "--working-dir", str(tmp_path)]
        )
